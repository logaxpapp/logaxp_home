...
import Pagination from './Pagination'; // A custom pagination component
import { useSelector } from 'react-redux';
import { RootState } from '../../app/store';
import { UserRole } from '../../types/enums';
import { Permissions, getPermissions } from '../../utils/permissions'; // Updated import
...
const ListTeams: React.FC = () => {
  ...
  const [selectedTeam, setSelectedTeam] = useState<any>(null); // Replace `any` with your Team type
  ...
  const openEditModal = (team: any) => { // Replace `any` with your Team type
  ...
  const openAddMemberModal = (team: any) => { // Replace `any` with your Team type
    setSelectedTeam(team);
  ...
  return (
    <div className=" bg-gray-50 min-h-screen py-6">
      ...
      {data?.teams?.length ? (
          ...
          {data.teams.map((team: any) => (
            <div key={team?._id} className="p-4 bg-gray-100 rounded-lg shadow">
              <div className="flex justify-between items-center mb-2">
                <Link to={`/dashboard/teams/${team?._id}`} className="text-2xl font-semibold text-teal-600 hover:underline">
        ...
        <p className="text-gray-600">No teams found. Click "Create Team" to get started.</p>
      )}
      ...
      {/* Add Member Modal */}
      <Modal isOpen={isAddMemberModalOpen} onClose={closeAddMemberModal} title="Add Member to Team">
        {selectedTeam && <AddMemberToTeam teamId={selectedTeam._id} onClose={closeAddMemberModal} />}
      </Modal>
