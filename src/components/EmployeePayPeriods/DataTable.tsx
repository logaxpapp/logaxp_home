...
export interface Column<T> {
  id: keyof T | string; // Allow `keyof T` or custom string IDs
  label: string;
...
const DataTable = <T extends { _id: string }>({
  ...
  const [orderBy, setOrderBy] = React.useState<keyof T | string>(columns[0].id);
      ...
      <table className="min-w-full divide-y divide-gray-200">
        ...
        <thead className="bg-gray-50 font-primary">
          <tr>
            {columns.map((column) => (
              <th
                key={String(column.id)} // Ensure the key is string
                scope="col"
