...
import { SpinnerProps } from './LoadingSpinner.types';
...
const LoadingSpinner: React.FC<SpinnerProps> = ({
  ...
  const colorClasses = {
    lemonGreen: 'text-lemonGreen-light',
    blue: 'text-blue-500',
    red: 'text-red-500',
    gray: 'text-gray-500',
  ...
  return (
    <div role="status" aria-label={ariaLabel} className="flex justify-center items-center">
      <svg
        className={`animate-spin ${sizeClasses[size]} ${colorClasses[color]} fill-current`}
