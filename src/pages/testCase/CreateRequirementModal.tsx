...
interface CreateRequirementModalProps {
  onClose: () => void;
  onCreate: (input: Partial<IRequirement>) => void;
...
export const CreateRequirementModal: React.FC<CreateRequirementModalProps> = ({
  ...
  const [application, setApplication] = useState<string>(defaultApplication);
  ...
  const handleSubmit = () => {
    ...
    setErrors({});
    onCreate({ title, description, priority, status, application });
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm">
      <div className="bg-white w-[90%] max-w-md p-6 rounded-lg shadow-xl transform transition-all duration-300 ease-in-out">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-xl font-semibold">Create Requirement</h3>
          <button
            onClick={onClose}
        ...
        <div className="mb-4">
          ...
          {errors.title && (
            <p className="text-red-500 text-sm mt-1 flex items-center">
              <FaAdjust className="h-4 w-4 mr-1" />
              {errors.title}
            </p>
          )}
        </div>
